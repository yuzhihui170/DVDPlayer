...
public class DVDMp5Vedio extends DVDEngineBase
{
    ...
    private static DVDMp5Vedio dvdMp5Video = null;
	private DVDMp5Vedio()
	{
		mdata = AppData.Instance();
	}
	
	public static DVDMp5Vedio Instance()
	{
		if(dvdMp5Video == null)
		{
			dvdMp5Video = new DVDMp5Vedio();
		}
	...
	{
		LogCat.Logd("DVDMp5Vedio -- Show");
		
		mContext = context;
		
		if(DVDMp5VideoActivity.DVDMp5VedioAct == null)
		{
			LogCat.Logd("DVDMp5Video act = null");
			Intent mp5vedio = new Intent(context, 
					com.apical.dvdplayer.dvdview.DVDMp5VideoActivity.class);
...
//			}
			...
			if(tasksInfo.size() > 0)
			{
				LogCat.Logd("SDHCMp5Video tasksInfo.size() > 0");
				//DVD 未置顶
				if( !packageName.equals(tasksInfo.get(0).topActivity.getPackageName()) )
				{
					LogCat.Logd("SDHCMp5Video not top");
					final ActivityManager am = (ActivityManager)
					           context.getSystemService(Context.ACTIVITY_SERVICE);
					am.moveTaskToFront(DVDMp5VideoActivity.DVDMp5VedioAct.getTaskId(), ActivityManager.MOVE_TASK_WITH_HOME);		
				}
			}
		}
	...
	public void TrgCompleted(Intent intent, int startId)
	{
        ...
        if(bundle != null) 
        {
        	...
        	LogCat.Logd("DVDMp5Vedio  trgCmd = " + trgCmd);
        	switch(trgCmd)
        	{
        ...
        {
        	LogCat.Logd("DVDMp5Vedio  bundle = null");
        }
	}
	...
	{
		LogCat.Logd("DVDMp5Vedio CmdCompleted cmd = " + cmd);
		switch(cmd)
		{
